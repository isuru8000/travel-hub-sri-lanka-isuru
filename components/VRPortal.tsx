...
const VRPortal: React.FC<VRPortalProps> = ({ language, setView }) => {
  ...
  if (isLocked) {
    return (
      <div className="min-h-screen bg-[#f5f5f0] text-[#2d2d2d] pt-24 pb-12 px-4 relative overflow-hidden flex items-center justify-center">
        {/* Background Elements */}
        <div className="absolute inset-0 z-0">
           <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1626387346567-2cd2dc03d3ce?q=80&w=2070&auto=format&fit=crop')] bg-cover bg-center opacity-10 blur-sm scale-110"></div>
           <div className="absolute inset-0 bg-gradient-to-t from-[#f5f5f0] via-[#f5f5f0]/90 to-transparent"></div>
        </div>
        ...
        <div className="relative z-50 flex flex-col items-center justify-center text-center max-w-2xl mx-auto p-8">
            <div className="w-24 h-24 rounded-full bg-[#5A5A40]/10 border border-[#5A5A40]/20 flex items-center justify-center mb-8 shadow-[0_0_60px_rgba(90,90,64,0.1)] animate-pulse">
                <Lock size={40} className="text-[#5A5A40]" />
            </div>
            
            <h1 className="text-4xl md:text-6xl font-serif font-bold mb-6 text-[#2d2d2d]">
                {language === 'EN' ? 'Portal Locked' : 'ද්වාරය අගුළු දමා ඇත'}
            </h1>
            
            <p className="text-lg md:text-xl text-[#5A5A40]/80 mb-10 font-light leading-relaxed">
                {language === 'EN' 
                    ? 'This immersive VR experience is currently under construction by our architects. Access will be granted soon.' 
                    : 'මෙම VR අත්දැකීම දැනට අපගේ ඉංජිනේරුවන් විසින් නිර්මාණය කරමින් පවතී. ළඟදීම ඔබට විවෘත වනු ඇත.'}
            </p>
            
            <div className="flex items-center gap-3 px-6 py-2.5 rounded-full bg-[#5A5A40]/10 border border-[#5A5A40]/20 text-[#5A5A40] font-mono text-xs md:text-sm uppercase tracking-[0.2em] mb-12">
            ...
            <button 
                ...
                className="px-10 py-4 bg-[#5A5A40] text-white rounded-full font-bold uppercase tracking-[0.2em] hover:scale-105 transition-transform shadow-lg"
            >
      ...
      <div className="min-h-screen bg-[#f5f5f0] text-[#2d2d2d] pt-24 pb-12 px-4 relative">
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="mb-12">
            ...
            <p className="text-[#5A5A40]/80 max-w-2xl text-lg">
              {language === 'EN' 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {vrTours.map(tour => (
              <div key={tour.id} className="bg-white border border-[#5A5A40]/10 rounded-[2rem] overflow-hidden hover:border-[#5A5A40]/30 transition-all group flex flex-col shadow-sm">
                ...
                <div className="p-6 flex flex-col flex-1">
                  <p className="text-sm text-[#5A5A40]/70 mb-6 line-clamp-3 flex-1">{tour.desc[language]}</p>
                  
                  <div className="flex items-center justify-between text-xs text-[#5A5A40]/50 mb-6">
          ...
          <div className="flex flex-col lg:flex-row gap-6 flex-1">
            ...
            <div className="flex-1 bg-[#2d2d2d] rounded-3xl overflow-hidden relative border border-[#5A5A40]/20 shadow-lg min-h-[50vh] lg:min-h-[70vh] group">
              ...
              <div className="absolute top-4 left-4 bg-[#f5f5f0]/80 backdrop-blur-md px-4 py-2 rounded-2xl border border-[#5A5A40]/20 opacity-0 group-hover:opacity-100 transition-opacity">
                <h2 className="font-bold text-lg text-[#2d2d2d]">{selectedTour.title[language]}</h2>
                <p className="text-xs text-[#5A5A40]/70">Guide: {selectedTour.guide}</p>
              </div>

              {/* VR Headset Hint */}
              <div className="absolute bottom-4 left-4 bg-[#f5f5f0]/80 backdrop-blur-md px-4 py-2 rounded-xl border border-[#5A5A40]/20 flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                <div className="w-8 h-8 rounded-full bg-[#5A5A40]/10 flex items-center justify-center">
                  <Eye size={16} className="text-[#5A5A40]" />
                </div>
                <div>
                  <p className="text-xs font-bold text-[#2d2d2d]">VR Headset Detected?</p>
                  <p className="text-[10px] text-[#5A5A40]/70">Click the VR icon on the video player to enter immersive mode.</p>
                </div>
              </div>
            </div>
