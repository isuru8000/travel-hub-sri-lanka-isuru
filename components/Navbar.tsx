...
const Navbar: React.FC<NavbarProps> = ({ 
  ...
  return (
    <nav className={`fixed top-4 md:top-6 left-1/2 -translate-x-1/2 z-[100] transition-all duration-500 w-[96%] md:w-[98%] max-w-7xl glass-card rounded-full shadow-[0_25px_60px_rgba(0,0,0,0.5)] ${isScrolled ? 'py-1 px-4 md:px-5' : 'py-2 px-4 md:py-2.5 md:px-8'}`}>
      <div className="flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-1">
            ...
            >
              ...
              {link.hasDropdown && activeDropdown === link.id && (
                <div className="absolute top-full left-1/2 -translate-x-1/2 w-64 pt-3 animate-in fade-in slide-in-from-top-2 duration-300">
                  <div className="glass-card rounded-[2.5rem] shadow-[0_40px_120px_rgba(0,0,0,0.5)] border border-white/10 overflow-hidden p-2">
                      ...
                      {link.items?.map((subItem) => (
                        <button
                          ...
                          className={`w-full flex items-center gap-4 px-5 py-3.5 rounded-2xl text-[10px] font-bold uppercase tracking-widest transition-all ${
                            currentView === subItem.id ? 'bg-[#F59E0B]/20 text-[#F59E0B]' : 'text-white/60 hover:bg-white/5 hover:text-white'
                          }`}
                        >
          ...
          <div className="flex items-center gap-4">
            <button 
              ...
              className="group flex items-center gap-3 px-5 py-2.5 rounded-2xl border border-white/10 text-[10px] font-black uppercase tracking-widest bg-white/5 text-white/60 hover:bg-[#F59E0B] hover:text-white hover:border-transparent transition-all shadow-sm"
              title={language === 'EN' ? "Switch to Sinhala Vision" : "ආපසු ඉංග්‍රීසි දැක්මට"}
            >
              <Globe size={13} className={`${language === 'EN' ? 'text-[#F59E0B]' : 'text-white'} group-hover:text-white transition-colors`} />
              {language === 'EN' ? 'Sinhala Vision' : 'English View'}
            </button>

            {user ? (
              <button onClick={() => handleNav('community')} className="w-10 h-10 rounded-full border-2 border-white/20 shadow-lg overflow-hidden group">
                 <img src={user.photo} className="w-full h-full object-cover group-hover:scale-110 transition-transform" />
              </button>
            ) : (
              <button onClick={onLogin} className="px-8 py-3 bg-white text-black rounded-full text-[10px] font-black uppercase tracking-[0.2em] hover:scale-105 active:scale-95 transition-all shadow-md">
        ...
        {/* Mobile Toggle */}
        <button className="lg:hidden p-2 md:p-3 rounded-xl md:rounded-2xl bg-white/10 border border-white/20 text-white" onClick={() => setIsMenuOpen(!isMenuOpen)}>
      ...
      {/* Mobile Drawer */}
      {isMenuOpen && (
        <div className="fixed inset-x-0 top-full mt-2 md:mt-4 bg-[#0a0a0a]/95 backdrop-blur-3xl rounded-[2rem] md:rounded-[3rem] shadow-[0_50px_100px_rgba(0,0,0,0.5)] border border-white/10 p-4 md:p-8 flex flex-col gap-4 md:gap-8 animate-in slide-in-from-top-4 duration-500 max-h-[85vh] overflow-y-auto w-full">
          {/* Header Identity in Mobile Menu */}
          <div className="flex items-center gap-3 md:gap-5 px-4 py-3 md:px-6 md:py-4 bg-white/5 rounded-[1.5rem] md:rounded-[2.5rem] shadow-sm shrink-0 border border-white/10">
             <AnimatedLogo />
             <div className="flex flex-col items-start leading-none">
                <span className="text-xs md:text-sm font-heritage font-bold text-white uppercase tracking-tighter">TRAVEL <span className="text-[#F59E0B]">HUB</span></span>
                <span className="text-[6px] md:text-[7px] font-black text-white/40 uppercase tracking-[0.2em] md:tracking-[0.3em]">sri lanka</span>
             </div>
          </div>
              ...
              <div key={link.id} className="space-y-1 md:space-y-3">
                ...
                >
                  <div className="flex items-center gap-3 md:gap-6">
                    <span className={currentView === link.id ? 'text-[#F59E0B]' : ''}>{link.icon}</span>
                      ...
                      <button
                        ...
                        className={`flex items-center gap-2 md:gap-4 p-3 md:p-5 rounded-xl md:rounded-2xl text-[9px] md:text-[10px] font-black uppercase tracking-[0.1em] md:tracking-[0.2em] transition-all border ${
                          currentView === subItem.id 
                            ? 'bg-[#F59E0B]/20 text-[#F59E0B] border-[#F59E0B]/30 shadow-sm' 
                            : 'bg-white/5 text-white/40 border-white/10 hover:text-white hover:bg-white/10 shadow-sm'
                        }`}
                      >
          ...
          <div className="pt-3 md:pt-6 border-t border-white/10 flex flex-col gap-3 md:gap-4 shrink-0">
             <div className="grid grid-cols-2 gap-2 md:gap-4">
                <button 
                  ...
                  className="py-3 md:py-6 rounded-[1.25rem] md:rounded-[2.5rem] bg-white/5 border border-white/10 text-[#F59E0B] font-black text-[9px] md:text-[10px] uppercase tracking-[0.2em] md:tracking-[0.3em] flex items-center justify-center gap-2 md:gap-4 hover:bg-white/10 transition-all shadow-sm"
                >
                  <Globe size={14} className="md:w-[18px] md:h-[18px] text-[#F59E0B]" />
                  {language === 'EN' ? 'Sinhala Vision' : 'English View'}
                </button>
                ...
                {user ? (
                  <button 
                    onClick={onLogout} 
                    className="py-3 md:py-6 bg-red-500/10 border border-red-500/20 text-red-500 rounded-[1.25rem] md:rounded-[2.5rem] font-black text-[9px] md:text-[10px] uppercase tracking-[0.2em] md:tracking-[0.3em] hover:bg-red-500/20 transition-all shadow-sm"
                  >
                  ...
                  <button 
                    onClick={onLogin} 
                    className="py-3 md:py-6 bg-white text-black rounded-[1.25rem] md:rounded-[2.5rem] font-black text-[9px] md:text-[10px] uppercase tracking-[0.2em] md:tracking-[0.3em] shadow-xl hover:scale-105 active:scale-95 transition-all"
                  >
             ...
             <div className="flex justify-center items-center gap-3 pt-2 opacity-40">
                <span className="text-[8px] md:text-[9px] font-black text-white/40 uppercase tracking-[0.4em] md:tracking-[0.6em]">
